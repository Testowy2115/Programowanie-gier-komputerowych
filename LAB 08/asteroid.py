...
class Asteroid:
    def __init__(self, position, level=3):
        ...
        self.level = level
        self.radius = 15 * level
        self.alive = True
        self.angle = 0.0
        self.rot_speed = random.uniform(-2.0, 2.0)
        
        speed = random.uniform(20 * (4 - level), 40 * (4 - level))
        move_angle = math.radians(random.uniform(0, 360))
        self.velocity = (math.cos(move_angle) * speed, math.sin(move_angle) * speed)
        
        num_points = 10 + level * 2
        self.local_vertices = []
        for i in range(num_points):
            angle_step = (i / num_points) * 2 * math.pi
            random_radius = self.radius * random.uniform(0.7, 1.3)
            self.local_vertices.append((math.cos(angle_step) * random_radius, math.sin(angle_step) * random_radius))

    def split(self):
        if self.level == 1:
            return []
        return [
            Asteroid(self.position, self.level - 1),
            Asteroid(self.position, self.level - 1)
        ]
